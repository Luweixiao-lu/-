...
def show_recognition_page():
    ...
    3. 将手放在摄像头前，做出手语字母手势
    4. 系统会**实时自动识别**并显示结果
    5. 支持识别30个手语字母（A-Z, ZH, CH, SH, NG）
    
    ⚠️ **注意**：这是真正的实时识别，无需点击拍照按钮！
    """)
    ...
    with col1:
        ...
        if ctx.video_processor:
            ...
            if processor.current_prediction:
                # 更新session state中的历史记录
                if (not st.session_state.prediction_history or 
                    processor.current_prediction != st.session_state.prediction_history[-1]):
                    st.session_state.prediction_history.append(processor.current_prediction)
    ...
    with col2:
        ...
        if ctx.video_processor and ctx.video_processor.current_prediction:
            prediction = ctx.video_processor.current_prediction
