...
确保包含所有必要的依赖（已针对Streamlit Cloud优化）：
...
**重要提示**：
- 使用 `opencv-python-headless` 而不是 `opencv-python`（服务器环境）
- `mediapipe` 使用范围版本 `>=0.10.0` 而不是固定版本
- 移除了 `pygame`（Web应用不需要）
...
1. 使用范围版本而不是固定版本：
   ...
   mediapipe>=0.10.0
   ```
2. 使用 `opencv-python-headless` 代替 `opencv-python`：
   ```txt
   opencv-python-headless>=4.8.0
   ```
3. 确保 `requirements.txt` 已更新并推送到GitHub
4. 在Streamlit Cloud中重新部署
