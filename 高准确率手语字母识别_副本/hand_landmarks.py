...
try:
    ...
    logger.error(f"导入numpy失败: {str(e)}")
    raise
...
try:
    ...
    # 捕获所有可能的导入错误，包括系统库缺失（OSError）
    logger.error(f"导入cv2失败: {str(e)}")
    cv2 = None
    CV2_AVAILABLE = False

try:
    import mediapipe as mp
    logger.info("成功导入mediapipe")
    MEDIAPIPE_AVAILABLE = True
except (ImportError, OSError, RuntimeError) as e:
    # 捕获所有可能的导入错误，包括系统库缺失（OSError）和运行时错误
    logger.error(f"导入mediapipe失败: {str(e)}")
    mp = None
...
class HandLandmarkDetector:
    ...
    def __init__(self):
        if not MEDIAPIPE_AVAILABLE or mp is None:
            logger.warning("MediaPipe不可用，HandLandmarkDetector将以受限模式运行")
            self.mp_hands = None
            self.hands = None
            ...
            try:
                ...
                self.available = True
                logger.info("HandLandmarkDetector初始化成功")
            except Exception as e:
                logger.error(f"初始化MediaPipe Hands失败: {str(e)}")
                self.mp_hands = None
                self.hands = None
                self.mp_drawing = None
                self.available = False
    ...
    def detect(self, image):
        ...
        try:
            ...
            logger.error(f"检测手部关键点时出错: {str(e)}")
            return None, image.copy() if image is not None else None
