...
try:
    ...
    if IN_STREAMLIT_CLOUD:
        st.warning("在Streamlit Cloud上无法使用摄像头功能，这是正常的限制。请在本地环境运行以体验完整功能。")
    else:
        st.warning("无法导入OpenCV库，可能无法使用摄像头功能。请确保已安装opencv-python包。")
...
try:
    ...
    logger.info("成功导入自定义模块")
except ImportError as e:
    logger.error(f"导入自定义模块失败: {str(e)}")
    error_msg = traceback.format_exc()
    logger.debug(error_msg)
    st.warning(f"无法加载必要的自定义模块: {str(e)}")
    st.info("请检查hand_landmarks.py和gesture_classifier.py文件是否存在且没有错误")
    
    # 创建一个简单的替代类，避免后续代码中的NoneType错误
    ...
    st.info("已创建替代类以避免应用崩溃，部分功能可能受限")
