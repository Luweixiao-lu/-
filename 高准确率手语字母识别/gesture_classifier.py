"""
手势分类器模块
使用预训练的机器学习模型对手部关键点进行分类
识别30个手语字母（A-Z, ZH, CH, SH, NG）
"""
import sys
import logging
import os
...
try:
    import pickle
    logger.info("成功导入pickle")
except ImportError as e:
    logger.error(f"导入pickle失败: {str(e)}")
    raise
...
try:
    ...
    logger.info("成功导入sklearn相关模块")
except ImportError as e:
    logger.error(f"导入sklearn模块失败: {str(e)}")
    raise
